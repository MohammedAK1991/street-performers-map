import type { Performance, CreatePerformanceDto } from '@spm/shared-types';
import { PerformanceModel, type PerformanceDocument } from '@/domains/performance/entities/Performance';
import { logger } from '@/shared/utils/logger';

export class PerformanceRepository {
  private readonly logger = logger.child({ context: 'PerformanceRepository' });

  async create(performanceData: Omit<Performance, '_id' | 'createdAt' | 'updatedAt'>): Promise<PerformanceDocument> {
    try {
      ...
      this.logger.error('Failed to create performance', { error, performanceData });
      throw error;
    }
  }
  ...
  async findById(id: string): Promise<PerformanceDocument | null> {
    try {
      ...
      this.logger.error('Failed to find performance by ID', { error, performanceId: id });
      throw error;
    }
  }

  async findByPerformerId(performerId: string, limit: number = 10): Promise<PerformanceDocument[]> {
    try {
      ...
      this.logger.error('Failed to find performances by performer', { error, performerId });
      throw error;
    }
  }
  ...
  async findNearby(
    ...
    try {
      ...
      const performances = await PerformanceModel.find(query)
        .populate('performerId', 'username profile.displayName profile.avatar')
        .sort({ 'engagement.likes': -1, scheduledFor: 1 })
        .limit(50)
        .lean();

      return performances;
    } catch (error) {
      this.logger.error('Failed to find nearby performances', { error, coordinates, radiusInKm });
      throw error;
    }
  }

  async updateById(id: string, updateData: Partial<Performance>): Promise<PerformanceDocument | null> {
    try {
      ...
      this.logger.error('Failed to update performance', { error, performanceId: id });
      throw error;
    }
  }
  ...
  async deleteById(id: string): Promise<boolean> {
    try {
      ...
      this.logger.error('Failed to delete performance', { error, performanceId: id });
      throw error;
    }
  }
  ...
  async incrementViews(performanceId: string): Promise<void> {
    try {
      ...
      this.logger.error('Failed to increment performance views', { error, performanceId });
      throw error;
    }
  }
