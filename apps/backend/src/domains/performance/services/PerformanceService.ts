...
export class PerformanceService {
	...
	async createPerformance(
		...
		try {
			...
			for (let i = 0; i < data.route.stops.length; i++) {
				...
				if (startTime >= endTime) {
					throw new ValidationError(
						`Stop ${i + 1}: Start time must be before end time`,
					);
				}
				...
				if (i < data.route.stops.length - 1) {
					...
					if (endTime > nextStartTime) {
						throw new ValidationError(
							`Stop ${i + 1} overlaps with stop ${i + 2}`,
						);
					}
				}
			}
			...
			const existingPerformances =
				await this.performanceRepository.findByPerformerId(performerId);
			const _todayPerformances = existingPerformances.filter((perf) => {
			...
			return performance as unknown as Performance;
		} catch (error) {
	...
	async getPerformanceById(id: string): Promise<Performance | null> {
		try {
			...
			return performance ? (performance as unknown as Performance) : null;
		} catch (error) {
			this.logger.error("Failed to get performance by ID", {
				error,
				performanceId: id,
			});
			throw error;
	...
	async getPerformancesByPerformer(
		performerId: string,
	): Promise<Performance[]> {
		try {
			const performances =
				await this.performanceRepository.findByPerformerId(performerId);
			return performances as unknown as Performance[];
		} catch (error) {
			this.logger.error("Failed to get performances by performer", {
				error,
				performerId,
			});
			throw error;
	...
	async getNearbyPerformances(
		...
		try {
			const performances = await this.performanceRepository.findNearby(
				coordinates,
				radiusInKm,
				filters,
			);
			...
			this.logger.error("Failed to get nearby performances", {
				error,
				coordinates,
				radiusInKm,
			});
	...
	async updatePerformance(
		...
		try {
			...
			const updatedPerformance = await this.performanceRepository.updateById(
				id,
				updateData as Partial<Performance>,
			);
			return updatedPerformance
				? (updatedPerformance.toJSON() as Performance)
				: null;
		} catch (error) {
	...
	async toggleLikePerformance(
		...
		try {
			...
			const existingPerformance =
				await this.performanceRepository.findById(performanceId);
			if (!existingPerformance) {
				throw new ApiError(404, "Performance not found");
			}
			...
			if (alreadyLiked) {
				...
				updatedPerformance = await this.performanceRepository.unlikePerformance(
					performanceId,
					userId,
				);
				...
				updatedPerformance = await this.performanceRepository.likePerformance(
					performanceId,
					userId,
				);
			...
			this.logger.error("Failed to toggle like performance", {
				error,
				performanceId,
				userId,
			});
	...
	async likePerformance(
		...
		try {
			...
			const updatedPerformance =
				await this.performanceRepository.likePerformance(performanceId, userId);
			return updatedPerformance
				? (updatedPerformance.toJSON() as Performance)
				: null;
		} catch (error) {
	...
	async unlikePerformance(
		...
		try {
			const updatedPerformance =
				await this.performanceRepository.unlikePerformance(
					performanceId,
					userId,
				);
			return updatedPerformance
	...
	async startPerformance(
		...
		try {
			...
			this.logger.error("Failed to start performance", {
				error,
				performanceId,
				performerId,
			});
	...
	async endPerformance(
		...
		try {
			...
			this.logger.error("Failed to end performance", {
				error,
				performanceId,
				performerId,
			});
	...
	async getTodaysPerformances(): Promise<Performance[]> {
		try {
			const performances =
				await this.performanceRepository.findTodaysPerformances();
			return performances as unknown as Performance[];
		} catch (error) {
	...
	async getLivePerformances(): Promise<Performance[]> {
		try {
			const performances =
				await this.performanceRepository.findLivePerformances();
			return performances as unknown as Performance[];
		} catch (error) {
	...
	async incrementViews(performanceId: string): Promise<void> {
		try {
			await this.performanceRepository.incrementViews(performanceId);
		} catch (error) {
			this.logger.error("Failed to increment views", { performanceId, error });
			throw error;
		}
	}
}
