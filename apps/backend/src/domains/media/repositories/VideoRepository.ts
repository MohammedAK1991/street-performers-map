import { Video, type VideoDocument } from "../entities/Video";
...
export class VideoRepository {
	 ...
	 */
	async findByUserId(userId: string, limit = 20): Promise<VideoDocument[]> {
	...
	async updateStatus(
		...
		return await Video.findByIdAndUpdate(videoId, { status }, { new: true });
	}
	 ...
	 */
	async findPendingModeration(limit = 50): Promise<VideoDocument[]> {
		return await Video.find({
			moderationStatus: "pending",
			status: "ready",
		})
