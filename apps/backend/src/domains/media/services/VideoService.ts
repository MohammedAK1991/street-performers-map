...
import type { VideoDocument } from "../entities/Video";
import {
...
export class VideoService {
	...
	async uploadVideo(request: UploadVideoRequest): Promise<UploadVideoResponse> {
		...
		if (!canUpload) {
			throw new Error(
				"Daily upload limit reached. You can only upload 5 videos per day.",
			);
		}
		...
		try {
			...
			const uploadResult: VideoUploadResult =
				await cloudinaryService.uploadVideo(file.buffer, {
					userId,
					performanceId,
			...
			const updatedVideo = await this.videoRepository.findById(
				(video._id as any).toString(),
			);
	 ...
	 * Get user's videos
	 */
	async getUserVideos(userId: string, limit = 20): Promise<VideoDocument[]> {
	...
	async saveClientUpload(videoData: any): Promise<VideoDocument> {
		...
		return (
			(await this.videoRepository.findById((video._id as any).toString())) ||
			video
		);
	}
	 ...
	 */
	async getVideosForModeration(limit = 50): Promise<VideoDocument[]> {
