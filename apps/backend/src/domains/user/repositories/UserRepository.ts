...
import { UserModel, type UserDocument } from '@/domains/user/entities/User';
import { logger } from '@/shared/utils/logger';

export class UserRepository {
  private readonly logger = logger.child({ context: 'UserRepository' });

  async create(userData: Omit<UserWithPassword, '_id' | 'createdAt' | 'updatedAt'>): Promise<UserDocument> {
    try {
      const user = new UserModel(userData);
      const savedUser = await user.save();
      
      this.logger.info('User created', { userId: savedUser._id, email: savedUser.email });
      return savedUser;
    } catch (error) {
  ...
  async findNearbyUsers(
    ...
    try {
      ...
      this.logger.error('Failed to find nearby users', { error, coordinates, radiusInKm });
      throw error;
    }
  }
  ...
  async checkUsernameExists(username: string): Promise<boolean> {
    try {
      ...
      this.logger.error('Failed to check username existence', { error, username });
      throw error;
    }
  }

  async incrementStatistic(userId: string, field: 'totalLikes' | 'totalTips' | 'performanceCount', amount: number = 1): Promise<void> {
    try {
      ...
      this.logger.error('Failed to increment user statistic', { error, userId, field, amount });
      throw error;
    }
  }
}
