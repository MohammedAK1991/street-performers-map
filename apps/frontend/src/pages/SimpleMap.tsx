...
import { useNearbyPerformances, useLikePerformance } from '@/hooks/usePerformances';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {showFilters && (
          <div className="mb-6 p-4 bg-white rounded-lg shadow border border-gray-200">
            <h3 className="font-semibold text-gray-900 mb-3">🎵 Filter by Genre</h3>
            <div className="flex flex-wrap gap-2">
              {['all', 'jazz', 'rock', 'folk', 'pop', 'classical'].map(genre => (
                <button
                  key={genre}
            ...
            <div className="flex">
              <div className="ml-3">
                <p className="text-sm text-blue-700">
                  <strong>Getting your location...</strong> Please allow location access to see nearby performances.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow border border-gray-200 mb-8 p-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-gray-900">🗺️ Live Performance Map</h2>
            <div className="text-sm text-gray-600">
              📍 {userLocation ? 'Your Location' : 'Madrid (Default)'} • 🎭 {filteredPerformances.length} performances nearby
            </div>
          </div>
          {isLoading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredPerformances.map(performance => {
            ...
            return (
              <div 
                key={performance._id} 
                className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow cursor-pointer"
                ...
                <div className="p-6">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right">
                      <p className="text-lg font-semibold text-gray-900">{performance.engagement.likes} ❤️</p>
                    </div>
                  </div>

                  {/* Performance Info */}
                  <h3 className="text-lg font-bold text-gray-900 mb-2">{performance.title}</h3>
                  <p className="text-sm text-gray-600 mb-2">Performance</p>
                  <p className="text-sm text-gray-500 mb-4">📍 {firstStop.location.address}</p>
                  
                  <div className="flex justify-between items-center text-sm text-gray-600 mb-4">
                    <span>🎵 {performance.genre.charAt(0).toUpperCase() + performance.genre.slice(1)}</span>
                    <span>⏰ {formatTime(firstStop.startTime)} - {formatTime(firstStop.endTime)}</span>
                  </div>
        ...
        {isSignedIn && (
          <div className="mt-12 text-center">
            <div className="bg-white rounded-lg shadow border border-gray-200 p-8">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-4">Ready to perform?</h3>
              <p className="text-gray-600 mb-6">Share your music with the world and build your audience!</p>
        ...
        {isSignedIn && filteredPerformances.length === 0 && (
          <div className="mt-12 text-center">
            <div className="bg-white rounded-lg shadow border border-gray-200 p-8">
              <div className="text-4xl mb-4">🎵</div>
              <h3 className="text-xl font-bold text-gray-900 mb-4">No performances found</h3>
              <p className="text-gray-600 mb-6">Try adjusting your filters or check back later for new performances!</p>
              <button 
