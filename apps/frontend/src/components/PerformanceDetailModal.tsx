...
export function PerformanceDetailModal({ performance, isOpen, onClose }: PerformanceDetailModalProps) {
        ...
        <div className="p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-2xl font-bold text-gray-900">{performance.engagement.likes} ❤️</p>
                  <p className="text-sm text-gray-500">{performance.engagement.views} views</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-600 capitalize">{performance.genre}</p>
                </div>
                <div>
                  <h3 className="font-semibold text-gray-900 mb-2">Duration</h3>
                  <p className="text-gray-600">{performance.route.stops.length} stop{performance.route.stops.length !== 1 ? 's' : ''}</p>
                </div>
              </div>

              {/* Next Stop */}
              {performance.status === 'scheduled' && performance.route.stops.length > 0 && (
                <div>
                  <h3 className="font-semibold text-gray-900 mb-2">Next Stop</h3>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <p className="font-medium text-gray-900">{performance.route.stops[0].location.address}</p>
                    <p className="text-sm text-gray-600">
                      {formatDate(performance.route.stops[0].startTime)} at {formatTime(performance.route.stops[0].startTime)}
                    </p>
                  </div>
                </div>
              )}
          ...
          {activeTab === 'route' && (
            <div className="space-y-4">
              <h3 className="font-semibold text-gray-900 mb-4">Performance Route</h3>
                ...
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-gray-900">{stop.location.address}</h4>
                      {stop.location.name && (
                        <p className="text-sm text-gray-600">{stop.location.name}</p>
                      )}
