...
export function GoogleMap({ 
  ...
  const apiKey = import.meta.env.VITE_GOOGLE_MAPS_API_KEY || 'AIzaSyBhmTGuHg2to9E4hJSuX-cJUkzgtVeYSuA';

  if (!apiKey) {
    return (
      <div className={`${className} bg-gradient-to-br from-blue-50 to-green-50 rounded-lg flex items-center justify-center`}>
        <div className="text-center">
          <div className="text-6xl mb-4">🗺️</div>
          <h3 className="text-2xl font-bold text-gray-900 mb-2">Google Maps API Key Required</h3>
          ...
          >
            <div className="p-3 max-w-sm">
              <div className="flex justify-between items-start mb-2">
                ...
                <span className="text-sm font-semibold text-gray-900">{selectedPerformance.engagement.likes} ❤️</span>
              </div>
              
              <h3 className="font-bold text-gray-900 mb-1">{selectedPerformance.title}</h3>
              <p className="text-sm text-gray-600 mb-1">Performance</p>
              <p className="text-sm text-gray-500 mb-2">📍 {selectedPerformance.route.stops[0].location.address}</p>
              
              <div className="flex justify-between items-center text-sm text-gray-600 mb-3">
                <span>🎵 {selectedPerformance.genre}</span>
                <span>⏰ {formatDateTime(selectedPerformance.route.stops[0].startTime)}</span>
              </div>
