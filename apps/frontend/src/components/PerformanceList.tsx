    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto">
        {performances.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No performances found</h3>
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <span className={`status-${performance.status === 'live' ? 'live' : performance.status === 'scheduled' ? 'scheduled' : 'soon'}`}>
                      {performance.status === 'live' ? '🔴 LIVE' : 
                       new Date(currentStop.startTime).getTime() - Date.now() < 30 * 60 * 1000 ? '🟡 SOON' : '🔵 TODAY'}
                    </span>
                    <div className="text-right">
                      <p className="text-sm font-semibold text-gray-900">{performance.engagement.likes} ❤️</p>
                      <p className="text-xs text-gray-500">{performance.engagement.views} views</p>
                    </div>
                  </div>

                  {/* Performance Info */}
                  <div className="mb-3">
                    <h3 className="font-semibold text-gray-900 mb-1">{performance.title}</h3>
                    <p className="text-sm text-gray-600 mb-2">{performance.description}</p>
      ...
      <div className="p-4 border-t border-gray-200 bg-gray-50">
        <p className="text-xs text-gray-500 text-center">
          {performances.length} performance{performances.length !== 1 ? 's' : ''} near you
        </p>
      </div>
    </div>
  );
