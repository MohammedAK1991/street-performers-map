	...
	return useQuery({
		...
		queryFn: async () => {
			try {
				...
				console.warn(
					"Upload eligibility endpoint not available, using defaults",
				);
				return {
					canUpload: true,
					remainingUploads: 10,
...
export const useVideoUpload = () => {
	...
	return useMutation({
		mutationFn: async ({
			file,
			performanceId,
		}: { file: File; performanceId?: string }) => {
	...
	return useMutation({
		mutationFn: async (videoData: any) => {
			const response = await api.post(
				"/media/videos/save-client-upload",
				videoData,
			);
			return response.data.data as Video;
		},
...
export const useLinkVideoToPerformance = () => {
	...
	return useMutation({
		mutationFn: async ({
			videoId,
			performanceId,
		}: { videoId: string; performanceId: string }) => {
			const response = await api.patch(
				`/media/videos/${videoId}/link-performance`,
				{
					performanceId,
				},
			);
